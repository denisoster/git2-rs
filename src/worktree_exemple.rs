...
impl Worktree {
    ...
    pub fn name(&self) -> Option<&str> {
        unsafe {
            ::opt_bytes(self, raw::git_worktree_name(self.raw)).and_then(|s| str::from_utf8(s).ok())
        }
    }
    ...
    pub fn path(&self) -> &Path {
        unsafe {
            util::bytes2path(::opt_bytes(self, raw::git_worktree_path(self.raw)).unwrap())
        }
    }
    ...
    pub fn validate(&self) -> Result<(), Error> {
        unsafe {
            call::try(raw::git_worktree_validate(call::convert(&self.raw)))?;
        }
        Ok(())
    }
    ...
    pub fn unlock(&self) -> Result<(), Error> {
        unsafe {
            call::try(raw::git_worktree_unlock(call::convert(&self.raw)))?;
        }
        Ok(())
    }
...
impl<'a> WorktreeAddOptions<'a> {
    ...
    pub fn new(reference: Option<Reference>) -> WorktreeAddOptions {
        WorktreeAddOptions {
            lock: 0,
            reference: reference
        }
    ...
    pub unsafe fn raw(&self) ->  raw::git_worktree_add_options {
        ...
        assert_eq!(raw::git_worktree_add_init_options(&mut opts, raw::GIT_WORKTREE_ADD_OPTIONS_VERSION), 0);
...
impl WorktreePruneOptions {
    ...
    pub unsafe fn raw(&self) -> raw::git_worktree_prune_options {
        let mut opts = mem::zeroed();
        assert_eq!(raw::git_worktree_prune_init_options(&mut opts, raw::GIT_WORKTREE_PRUNE_OPTIONS_VERSION), 0);
...
impl Binding for Worktree {
    ...
    fn raw(&self) -> *mut raw::git_worktree { self.raw }
}
...
#[cfg(test)]
mod tests {
    use WorktreeAddOptions;
    use WorktreeLockStatus;
    use tempdir::TempDir;

    #[test]
    fn smoke_add_no_ref() {
        let (_td, repo) = ::test::repo_init();
        let wtdir = TempDir::new("worktrees").unwrap();
        ...
        assert_eq!(wt.path().canonicalize().unwrap(), wt_path.canonicalize().unwrap());
    ...
    fn smoke_add_locked() {
        ...
        assert_eq!(wt.is_locked().unwrap(), WorktreeLockStatus::Locked(Some("my reason".to_string())));
    }

    #[test]
    fn smoke_add_from_branch() {
        let (_td, repo) = ::test::repo_init();
        let (wt_top, branch) = ::test::worktrees_env_init(&repo);
